...
void RenderWindowPT::initialize() {
  ...
  if (this->currentScene.hasEnvLight) {
    ...
    this->envTextureBuffer =
        owlTexture2DCreate(context, OWL_TEXEL_FORMAT_RGBA32F, resx, resy,
                           envMap, OWL_TEXTURE_LINEAR, OWL_TEXTURE_CLAMP);
    this->envWidth = resx;
    this->envHeight = resy;

    this->conditionalPdf =
        owlTexture2DCreate(context, OWL_TEXEL_FORMAT_R32F, resx + 1, resy, cPdf,
                           OWL_TEXTURE_NEAREST, OWL_TEXTURE_CLAMP);

    this->conditionalCdf =
        owlTexture2DCreate(context, OWL_TEXEL_FORMAT_R32F, resx + 1, resy, cCdf,
                           OWL_TEXTURE_NEAREST, OWL_TEXTURE_CLAMP);

    this->marginalPdf =
        owlTexture2DCreate(context, OWL_TEXEL_FORMAT_R32F, resy + 1, 1.f, mPdf,
                           OWL_TEXTURE_NEAREST, OWL_TEXTURE_CLAMP);

    this->marginalCdf =
        owlTexture2DCreate(context, OWL_TEXEL_FORMAT_R32F, resy + 1, 1.f, mCdf,
                           OWL_TEXTURE_NEAREST, OWL_TEXTURE_CLAMP);
  ...
  this->gBuffer = owlDeviceBufferCreate(context, OWL_USER_TYPE(DenoiseGBuffer),
                                        frameSize, nullptr);
  this->fittedGBuffer = owlDeviceBufferCreate(
      context, OWL_USER_TYPE(DenoiseGBuffer), frameSize, nullptr);
  // ====================================================
  ...
  this->dLightsBuffer =
      owlDeviceBufferCreate(context, OWL_USER_TYPE(DirectionalLight),
                            this->dLightList.size(), this->dLightList.data());
  ...
  if (this->currentScene.has_surface) {
    ...
    for (auto mesh : surface->meshes) {
      OWLVarDecl triangleGeomVars[] = {
          ...
          {"diffuse_texture", OWL_TEXTURE,
           OWL_OFFSETOF(TriangleMeshData, diffuse_texture)},
          {"hasDiffuseTexture", OWL_BOOL,
           OWL_OFFSETOF(TriangleMeshData, hasDiffuseTexture)},

          {"alpha", OWL_FLOAT, OWL_OFFSETOF(TriangleMeshData, alpha)},
          {"alpha_texture", OWL_TEXTURE,
           OWL_OFFSETOF(TriangleMeshData, alpha_texture)},
          {"hasAlphaTexture", OWL_BOOL,
           OWL_OFFSETOF(TriangleMeshData, hasAlphaTexture)},

          {nullptr}};

      OWLGeomType triangleGeomType =
          owlGeomTypeCreate(context, OWL_GEOM_TRIANGLES,
                            sizeof(TriangleMeshData), triangleGeomVars, -1);

      owlGeomTypeSetClosestHit(triangleGeomType, RADIANCE_RAY_TYPE, module,
                               "triangleMeshCH");
      owlGeomTypeSetAnyHit(triangleGeomType, SHADOW_RAY_TYPE, module,
                           "triangleMeshAHShadow");
      owlGeomTypeSetAnyHit(triangleGeomType, MULTISCATTER_RAY_TYPE, module,
                           "triangleMeshAHMultiScatter");

      OWLGeom triangleGeom = owlGeomCreate(context, triangleGeomType);

      OWLBuffer vertexBuffer = owlDeviceBufferCreate(
          context, OWL_FLOAT3, mesh->vertex.size(), mesh->vertex.data());
      OWLBuffer vertexBuffer2 = owlDeviceBufferCreate(
          context, OWL_FLOAT3, mesh->vertex.size(), mesh->vertex.data());
      OWLBuffer normalBuffer = owlDeviceBufferCreate(
          context, OWL_FLOAT3, mesh->normal.size(), mesh->normal.data());
      OWLBuffer indexBuffer = owlDeviceBufferCreate(
          context, OWL_INT3, mesh->index.size(), mesh->index.data());
      OWLBuffer texCoordBuffer = owlDeviceBufferCreate(
          context, OWL_FLOAT2, mesh->texcoord.size(), mesh->texcoord.data());
      ...
      if (mesh->diffuseTextureID != -1) {
        ...
        OWLTexture diffuseTextureBuffer = owlTexture2DCreate(
            context, OWL_TEXEL_FORMAT_RGBA8, diffuseTexture->resolution.x,
            diffuseTexture->resolution.y, diffuseTexture->pixel,
            OWL_TEXTURE_NEAREST, OWL_TEXTURE_MIRROR);
        owlGeomSetTexture(triangleGeom, "diffuse_texture",
                          diffuseTextureBuffer);
        owlGeomSet1b(triangleGeom, "hasDiffuseTexture", true);
      } else {
        owlGeomSet3f(triangleGeom, "diffuse",
                     owl3f{mesh->diffuse.x, mesh->diffuse.y, mesh->diffuse.z});
      ...
      if (mesh->alphaTextureID != -1) {
        ...
        OWLTexture alphaTextureBuffer = owlTexture2DCreate(
            context, OWL_TEXEL_FORMAT_RGBA8, alphaTexture->resolution.x,
            alphaTexture->resolution.y, alphaTexture->pixel,
            OWL_TEXTURE_NEAREST, OWL_TEXTURE_MIRROR);
      ...
      owlTrianglesSetVertices(triangleGeom, vertexBuffer, mesh->vertex.size(),
                              sizeof(vec3f), 0);
      owlTrianglesSetIndices(triangleGeom, indexBuffer, mesh->index.size(),
                             sizeof(vec3i), 0);
      ...
      OWLGroup triangleGroup =
          owlTrianglesGeomGroupCreate(context, 1, &triangleGeom);
  ...
  // ====================================================
  OWLVarDecl hairGeomVars[] = {{nullptr}};

  OWLGeomType hairGeomType = owlGeomTypeCreate(
      context, OWL_GEOMETRY_CURVES, sizeof(HairData), hairGeomVars, -1);
  ...
  owlGeomTypeSetAnyHit(hairGeomType, MULTISCATTER_RAY_TYPE, module,
                       "hairAHMultiScatter");
  ...
  if (this->currentScene.has_hair) {
    ...
    this->sceneScale =
        max(this->currentScene.hairModel.scale, this->sceneScale);

    OWLBuffer cpBuffer = owlDeviceBufferCreate(
        context, OWL_FLOAT3, this->currentScene.hairModel.controlPoints.size(),
        this->currentScene.hairModel.controlPoints.data());
    OWLBuffer widthsBuffer = owlDeviceBufferCreate(
        context, OWL_FLOAT, this->currentScene.hairModel.widths.size(),
        this->currentScene.hairModel.widths.data());
    OWLBuffer indicesBuffer = owlDeviceBufferCreate(
        context, OWL_INT, this->currentScene.hairModel.segmentIndices.size(),
        this->currentScene.hairModel.segmentIndices.data());

    owlCurvesSetControlPoints(this->hairGeom,
                              this->currentScene.hairModel.controlPoints.size(),
                              cpBuffer, widthsBuffer);
    owlCurvesSetSegmentIndices(
        this->hairGeom, this->currentScene.hairModel.segmentIndices.size(),
        indicesBuffer);
  ...
  OWLVarDecl launchParamsDecl[] = {
      ...
      {"conditionalPdf", OWL_TEXTURE,
       OWL_OFFSETOF(LaunchParams, conditionalPdf)},
      {"conditionalCdf", OWL_TEXTURE,
       OWL_OFFSETOF(LaunchParams, conditionalCdf)},
      ...
      {"hasDirectionalLights", OWL_BOOL,
       OWL_OFFSETOF(LaunchParams, hasDirectionalLights)},
      ...
      {"num_total_lights", OWL_INT,
       OWL_OFFSETOF(LaunchParams, num_total_lights)},
    ...
    {"targetSpp", OWL_INT, OWL_OFFSETOF(LaunchParams, targetSpp)},
      ...
      {"averageBuffer", OWL_RAW_POINTER,
       OWL_OFFSETOF(LaunchParams, averageBuffer)},
      {"denoiseBuffer", OWL_RAW_POINTER,
       OWL_OFFSETOF(LaunchParams, denoiseBuffer)},
      ...
      {"fittedGuffer", OWL_BUFPTR, OWL_OFFSETOF(LaunchParams, fittedGuffer)},
      {nullptr}};

  this->launchParams =
      owlParamsCreate(context, sizeof(LaunchParams), launchParamsDecl, -1);

  // Global hair parameters
  owlParamsSet3f(this->launchParams, "sig_a",
                 owl3f{this->currentScene.sig_a.x, this->currentScene.sig_a.y,
  ...
  owlParamsSet3f(this->launchParams, "maxBound",
                 owl3f{this->maxBound.x, this->maxBound.y, this->maxBound.z});
  owlParamsSet3f(this->launchParams, "minBound",
  ...
  owlParamsSet1ul(this->launchParams, "accumBuffer",
                  (uint64_t)this->accumBuffer);
  owlParamsSet1ul(this->launchParams, "averageBuffer",
                  (uint64_t)this->averageBuffer);
  owlParamsSet1ul(this->launchParams, "denoiseBuffer",
                  (uint64_t)this->denoisedBuffer);
  owlParamsSetBuffer(this->launchParams, "gBuffer", this->gBuffer);
  owlParamsSetBuffer(this->launchParams, "fittedGuffer", this->fittedGBuffer);
  ...
  owlParamsSet1b(this->launchParams, "envPdfSampling",
                 this->currentScene.envPdfSampling);
  owlParamsSet1b(this->launchParams, "hasEnvLight",
                 this->currentScene.hasEnvLight);
  if (this->currentScene.hasEnvLight) {
    owlParamsSetTexture(this->launchParams, "env", this->envTextureBuffer);

    owlParamsSetTexture(this->launchParams, "conditionalPdf",
                        this->conditionalPdf);
    owlParamsSetTexture(this->launchParams, "conditionalCdf",
                        this->conditionalCdf);
  ...
  owlParamsSet1b(this->launchParams, "hasDirectionalLights",
                 this->currentScene.hasDirectionalLights);
  ...
  missProg = owlMissProgCreate(context, module, "miss", sizeof(MissProgData),
                               missProgVars, -1);
  ...
  this->rayGen = owlRayGenCreate(context, module, "rayGenCam",
                                 sizeof(RayGenData), rayGenVars, -1);
  this->fitGBufferPass = owlRayGenCreate(context, module, "fitGBuffer",
                                         sizeof(RayGenData), rayGenVars, -1);
  this->denoise = owlRayGenCreate(context, module, "denoise",
                                  sizeof(RayGenData), rayGenVars, -1);
...
long long RenderWindowPT::render() {
  ...
  if (this->accumId < this->currentScene.spp || this->progressive) {
    owlParamsSet1i(this->launchParams, "accumId", this->accumId);
    owlLaunch2D(this->rayGen, this->fbSize.x, this->fbSize.y,
                this->launchParams);
  ...
  if (fitGBuffer) {
    owlLaunch2D(this->fitGBufferPass, this->fbSize.x, this->fbSize.y,
                this->launchParams);

    cudaMemcpy(this->gBuffer, this->fittedGBuffer,
...
void RenderWindowPT::drawUI() {
...

  {
    ...
    bool fitGBuffer = this->fitGBuffer;
    ImGui::Checkbox("Fit GBuffer", &fitGBuffer);
    if (fitGBuffer != this->fitGBuffer) {
      this->fitGBuffer = fitGBuffer;
      this->cameraChanged();
    }

    bool denoiseEnabled = this->denoiseEnabled;
    ...
    ImGui::SliderFloat("tangent sigma", &this->tangentSigma, 0.f, 10.f);
    ImGui::SliderFloat("color sigma", &this->colorSigma, 0.f, 10.f);
    ImGui::SliderFloat("theta range", &this->thetaRange, 0.f, 3.14159f / 2.f);
   ...
   //  
    ...
    if (ImGui::CollapsingHeader("Hair")) {
      float cur_sig_a[3] = {this->currentScene.sig_a.x,
                            this->currentScene.sig_a.y,
                            this->currentScene.sig_a.z};
      ...
      if (this->currentScene.sig_a.x != cur_sig_a[0] ||
        ...
        this->currentScene.sig_a =
            vec3f(cur_sig_a[0], cur_sig_a[1], cur_sig_a[2]);
        owlParamsSet3f(this->launchParams, "sig_a",
                       owl3f{cur_sig_a[0], cur_sig_a[1], cur_sig_a[2]});
        this->cameraChanged();
      }
    ...
    if (ImGui::CollapsingHeader("Light")) {
      ...
      if (ImGui::TreeNode("Directional Lights")) {
        ...
        for (int i = 0; i < this->num_dlights; i++) {
          float dLightFrom[3] = {this->dLightList[i].from.x,
          ...
          if (dLightFrom[0] != this->dLightList[i].from.x ||
            ...
            this->dLightList[i].from =
                vec3f(dLightFrom[0], dLightFrom[1], dLightFrom[2]);
            this->uploadLights();
          }

          float emit[3] = {this->dLightList[i].emit.x,
    ...
    if (ImGui::CollapsingHeader("Path tracing")) {
      ...
      if (v1 != this->currentScene.path_v1 ||
          v2 != this->currentScene.path_v2) {
...
void RenderWindowPT::screenShotEXR(std::string fname) {
  ...
  cudaMemcpy(hostBuffer, this->averageBuffer, bufferSize,
...
RenderWindowPT::RenderWindowPT(Scene &scene, vec2i resolution, bool interactive)
  ...
  this->camera.setOrientation(
      scene.camera.from, scene.camera.at, scene.camera.up,
  ...
  owlEnableCurves(
      context); // Required since hair strands are represented by curves
}

void RenderWindowPT::customKey(char key, const vec2i &pos) {
  if (key == '1' || key == '!') {
    this->camera.setOrientation(this->camera.getFrom(), vec3f(0.f),
                                vec3f(0.f, 0.f, 1.f),
                                this->camera.getFovyInDegrees());
    this->cameraChanged();
  } else if (key == 'R' || key == 'r') {
    SceneCamera cam;
...
void RenderWindowPT::cameraChanged() {
  ...
  owlRayGenSet1ul(this->fitGBufferPass, "frameBuffer",
                  (uint64_t)this->fbPointer);
  owlRayGenSet2i(this->fitGBufferPass, "frameBufferSize",
                 (const owl2i &)this->fbSize);
  owlRayGenSet1ul(this->denoise, "frameBuffer", (uint64_t)this->fbPointer);
  owlRayGenSet2i(this->denoise, "frameBufferSize", (const owl2i &)this->fbSize);

  owlParamsSet3f(this->launchParams, "camera.pos", (const owl3f &)camera_pos);
  owlParamsSet3f(this->launchParams, "camera.dir_00",
                 (const owl3f &)camera_d00);
  owlParamsSet3f(this->launchParams, "camera.dir_du",
                 (const owl3f &)camera_ddu);
  owlParamsSet3f(this->launchParams, "camera.dir_dv",
                 (const owl3f &)camera_ddv);
